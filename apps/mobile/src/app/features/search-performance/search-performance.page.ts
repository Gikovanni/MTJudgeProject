...
export class SearchPerformancePage {
  ...
  private toDashboard(entries: QueryHistoryEntry[]): DashboardData { const durations = entries.map((entry: QueryHistoryEntry) => entry.durationMs ?? 0).filter((value) => value >= 0).sort((a, b) => a - b); const averageMs = durations.length ? durations.reduce((sum, value) => sum + value, 0) / durations.length : 0; const medianMs = durations.length ? durations[Math.floor(durations.length / 2)] : 0; return { averageMs, medianMs, noResultRate: entries.length ? entries.filter((entry: QueryHistoryEntry) => entry.totalResults === 0).length / entries.length * 100 : 0, total: entries.length, topQueries: this.count(entries.map((entry: QueryHistoryEntry) => entry.query)), topRules: this.count(entries.flatMap((entry: QueryHistoryEntry) => entry.selectedRuleId ? [`Regra ${entry.selectedRuleId}`] : [])), entries: entries.slice(0, 10) }; }
